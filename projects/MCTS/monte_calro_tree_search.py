...
class Node(ABC):
    ...
    @abstractmethod
    def __hash__(self):             # 노드에 해시 적용 가능하도록(hashable) 함
        return 123456789
...
class MCTS:
    ...
    def choose(self, node: Node):
        ...
        def score(n):                   # 점수 계산
            if self.N[n] == 0:
                return float('-inf')    # 한번도 방문하지 않은 노드는 선택 배제
    ...
    def _select(self, node: Node):      # 선택 단계
        '''
        ...
        while True:
            ...
            if node not in self.children or not self.children[node]:
                # node의 child나 grandchild가 아닌 경우
                # > 아직 시도해보지 않은 것 또는 단말 노드
                return path
    ...
    def _expand(self, node: Node): # 확장 단계
        '''
    ...
    def _simulate(self, node: Node): # 시뮬레이션 단계
        '''
    ...
    def _backpropagate(self, path, reward): # 역전파 단계
        ...
        for node in reversed(path): # 역순으로 가면서 Monte Carlo 시뮬레이션 결과 반영
            self.N[node] += 1
            self.Q[node] += reward
            reward = 1 - reward     # 자신은 1, 상대는 0 또는 그 반대
    ...
    def _uct_select(self, node: Node):
        ...
        def uct(n): # UCB(Upper Confidence Bound) 점수 계산
...
# from monte_calro_tree_search import MCTS, Node

TTTB = namedtuple("TicTacToeBoard", "tup turn winner terminal")

class TicTacToeBoard(TTTB, Node): # TTTB의 속성도 상속
    ...
    def reward(board):            # 점수 계산
        ...
        if board.winner is board.turn:
            ...
            raise RuntimeError(f'reward called on unreachalbe board {board}')
...
def play_game(): # 게임하기
    ...
    while True:
        ...
        if board.tup[index] is not None: # 비어있는 위치가 아닌 경우
            raise RecursionError('Invalid move')
        ...
        board = tree.choose(board)      # 최선의 값을 갖는 move를 선택하여 보드에 반영
        print(board.display_board())
        if board.terminal:
            print('게임 종료')
            break
